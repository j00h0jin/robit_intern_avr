...
아트메가128이 마스터 또는 다른 마스터의 slave도 될 수 있다.
...
# 232 통신과 485 통신에 대한 장단점



## RS-232
> 직렬 전송 방식을 사용한다. (데이터를 1비트씩 차례대로 보내는 방식)  
> 최장 15m까지 통신 가능하며 1대1 연결만 사용한다.
> 단방향 또는 전이중 통신이 가능하다.

### 장점

저비용으로 구축이 가능하다. (단거리 통신일 때 저비용으로 사용하기에 최적)
...
단방향 통신 방식이라 양방향 통신 시 2개의 전선이 필요하다.

전송 거리가 짧다.

노이즈에 취약하다. (단방향 통신 방식이기 때문에)


## RS-485
> 차동 신호 방식을 사용한다. (두 값의 차이를 봄)  
> 2 Wire과 4 Wire 방식이 있다. (2 Wire는 반이중 방식 , 4 Wire는 전이중 통신이 가능하다)

### 장점

장거리 통신이 가능하다. (최대 1200m)

상대적으로(232에 비해) 전송 속도가 빠르다.

여러 개의 장치를 연결할 수 있다.

노이즈에 강하다. (차를 구하는 과정에서 노이즈가 소거되는 효과가 있음(A - B = (A+NOISE) - (B + NOISE))
...
| 1대1 전용 | 1대다 지원 |
| 구조가 간단함 | 상대적으로 구조 복잡 |
| 최대 전송 거리가 짧음(~15m) | 비교적 먼 거리까지 전송 가능(~1200m) | 
| 저비용으로 구현 가능 | 비교적 구현에 비용이 듦 |


결론: 단거리에서 전송 속도가 크게 중요하지 않은 기능일 경우에는 저비용인 232를 사용, 그 외 경우에는 485를 사용하는 것이 더 유리함.
